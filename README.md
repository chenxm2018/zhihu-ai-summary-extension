...
为知乎文章、问题、回答添加AI总结功能，调用 ChatGPT/OpenAI/LongCat 兼容 API 进行智能总结。
...
- ⚡ **LongCat 快速接入**：支持一键添加 LongCat 默认账号
...
首次使用需要配置 OpenAI/LongCat 兼容 API：
...
#### 一键添加（推荐）
1. 打开知乎任意页面，点击右下角 **⚙️ 设置按钮**
2. 在“账号管理”页点击 **⚡ 一键添加 LongCat**
3. 插件会自动设置为当前账号，并可立即进行连接测试和总结

#### 手工配置
- **API接口地址**：`https://api.longcat.chat/openai/v1/chat/completions`
- **API Key**：你的 LongCat Key（格式通常为 `ak_...`）
- **模型名称**：`LongCat-Flash-Chat`（默认，可按实际可用模型调整）

#### 常见错误排查
- **401**：API Key 无效、过期或格式错误（检查是否携带 `Bearer`）
- **429**：请求限流或配额不足（稍后重试或调整调用频率）
- **网络错误/Failed to fetch**：检查 endpoint 是否正确、网络/代理是否可用
- **404**：接口路径错误，应使用 `/openai/v1/chat/completions`
...
- Optimization of core functions and performance improvements
...
### v1.3.0 (2026-02-06)
- 新增AI追问的"双模式"切换功能
- 支持"严格模式"（仅基于文章回答）和"自由模式"（可使用通用知识）
- 优化追问对话的UI交互体验
